...
//on 20-01-2020 by Amol	
    ...
    public function set_my_buffer($pdf_file = null){
            $this->buffer = $pdf_file;
            
            $this->bufferlen = strlen($this->buffer);        
    }
    
    public function my_updated_pdf($name = 'doc.pdf', $dest = 'I', $pdf_file = null, $text_add='', $sign_pos=0) {
        ...
        if(!empty($text_add)){
            ...
            $this->my_set_sign('', '', '', '', 2, $info); //custom function TCPDF  library tcpdf.php
            ...
            $noOfTotalPages = $this->getNumPages();
            
            echo $noOfTotalPages;
            die;
            
            for ($pagei = 1; $pagei <= $noOfTotalPages; $pagei++) {
                
                $this->setPage($pagei);
            
                // Draw a visible signature box at bottom right of each page
                $x = 17; // X-position
                $y = 250; // Y-position (adjust as needed)
                $width = 40;
                $height = 10;
            
                // Optional: Draw a rectangle and label
                //$this->SetXY($x, $y + 7);
                //$this->Rect($x, $y, $width, $height);
                $this->SetFont('helvetica', '', 8);
                //$this->setSignatureAppearance(5, 255, 30, 17, $pagei, 'Jeeva');
                $this->MultiCell($width, $height, 'Digitally Signed by: Jeeva '  . "\n" . 'Date: ' . date('d-m-Y') . "\n" . date('h:i a'), 0, '', 0, 1, $x, 255, true);
                $this->addEmptySignatureAppearance(10, 255, 60, 17, $pagei, 'Jeeva');
                
                
            }
            $this->setSignatureAppearance(10, 255, 60, 17); //X,Y,Width,Height            
        }        
