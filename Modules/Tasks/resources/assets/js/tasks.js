...
$(function () {
  ...
  Swal.fire({
    ...
    if (result.value) {
         $.ajax({
                url: APP_URL +'/tasks/tasks-delete/'+ dataId, 
                type: 'get', 
    ...
    $(document).on("click", "#clearMyActions", function () {
      document.cookie = 'myFilterTasks' + '=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;'
      window.location.href=`${APP_URL}/tasks`;
    });
    
    $(document).on("click", "#clearAllActions", function () {
      let userName = $(this).data('user');
      _setCookie_('myFilterTasks', userName);
      window.location.href=`${APP_URL}/tasks`;
    });
  ...
  if (dt_filter_table.length) {
    ...
    var dt_filter = dt_filter_table.DataTable({
      ...
      ajax: APP_URL + '/tasks/data?status=' + $('#status').val(),
      columns: [
        ...
        { data: 'end_date' },
        {
           ...
           {
            var button = '<a href="' + APP_URL +'/tasks/tasks-edit/' + data.id+'" class="btn btn-sm btn-icon dropdown-toggle hide-arrow text-body" data-bs-toggle="tooltip" title="Preview"><i class="ti ti-eye mx-2 ti-sm"></i></a>';

            
            var invoice_id = data.id;

                var button2 ='<div class="d-inline-block">' +
          '<a href="' + APP_URL +'/tasks/tasks-edit/' + data.id+'" class="btn btn-sm btn-icon dropdown-toggle hide-arrow" data-bs-toggle="dropdown"><i class="ti ti-dots-vertical"></i></a>' +
          '<div class="dropdown-menu dropdown-menu-end m-0">' +
          '<a href="' + APP_URL +'/tasks/tasks-edit/' + data.id+'" class="dropdown-item">Edit</a>' +
