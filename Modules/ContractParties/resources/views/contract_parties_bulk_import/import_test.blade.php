...
@section('title', ' Contracts Import Check')
<!-- Vendor Styles -->
    ...
    <div class="container network_wrapper col-sm p-2">
        ...
        <div class="card">
            <div class="card-header">
                <h5 class="card-title">Bulk Check Contract Parties</h5>
                <ul class="nav nav-tabs card-header-tabs" id="myTab" role="tablist">
                    ...
                    <!--</li>-->
                    <li class="nav-item" role="presentation">
                        <button class="nav-link {{ (session('data')) ? 'active' : 'active' }}" id="verify-info-tab" data-bs-toggle="tab" data-bs-target="#verify-info"
                            type="button" role="tab" aria-controls="verify-info" aria-selected="false">Upload &
                            Verify Info</button>
                    </li>
                </ul>
            </div>
            <div class="card-body">
                <div class="tab-content">
                    ...
                    <!--            <form id="createcontract" action="template-download-org-parties" method="POST"-->
                    ...
                    <!--</div>-->
                    <div class="tab-pane fade {{ (session('data')) ? 'show active' : 'show active' }}" id="verify-info" role="tabpanel" aria-labelledby="verify-info-tab">

                        <div class="row my-4">
                            <div class="col">
                                <h5 class="card-title">Upload Data</h5>
                                <form id="createcontract" action="{{ url('parties/builk-parties/check') }}"
                                    method="POST" enctype="multipart/form-data">
                                    @csrf
                                    <select class="form-select select2 partycontracttype userBranch" name="partyLocation">
                                        <option value="">-Select-</option>
                                        @foreach ($branchsUser as $branch)
                                            <option value="{{ $branch->id }}" {{ ($branch->id == ($contractParty['location'] ?? 0)) ? 'selected' : '' }}>{{ $branch->LegalName}}</option>
                                        @endforeach
                                    </select>
                                    <div class="mb-3">
                                        ...
                                        foreach (session('data') as $sheetName => $sheetData){
                                            if(strtolower($sheetName)){
                                                ...
                                                foreach (array_slice($sheetData, 1) as $row){
                                                    ...
                                                    if (array_filter($row)){
                                                    $sheetBodyHtml .="<tr>";
                                                        if(isset($dataample[$row['2'].$row['4']])){
                                                        $totalRows++;
                                            ...
                                            <div class="col-10">
                                                <span class="alert alert-danger">
                                                    ...
                                                    Invalid Sheet Name ('.$sheetName.') Please use sheet name as <b class="emptyattachemnt">Organization</b>
                                                </span>
                                            </div>
                                        </div>';
                                            }
