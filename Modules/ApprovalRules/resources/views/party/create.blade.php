...
@section('title', 'Party Approver Rules')
    ...
    <div class="container">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3">
           <div class="d-flex flex-column justify-content-center">
              <h4 class="mb-1">Add New Party Approver Rules</h4>
           </div>
      ...
      <div class="row align-items-center">
        ...
        <div class="col">
          <form class="row" id="financial_form" action="{{url('/')}}/contract-setup/party-approval-add" method="POST" enctype="multipart/form-data">
            @csrf
                                               ...
                                               <select class="form-select select2 contracttype"
                                                ...
                                                <option value="0">Any / All</option>
                                                  @foreach($branch as $branch_data)
                                                      <option value="{{$branch_data->id}}" {{ (old('location') == $branch_data->id) ? 'selected' : '' }}>{{$branch_data->BranchName}}</option>
                                                  @endforeach
                                              </select>
                                            </div>
                                            <div class="col-md-6 financial_form mt-2">
                                              <label for="access_level" class="form-label required">Access Level</label>
                                                <select class="form-select select2" aria-label="select example" id="access_level" name="access_level">
                                                  ...
                                                  @foreach($geo_graph as $geo)
                                                      <option value="{{$geo->id}}" {{ $geo->id == 1 ? 'selected' : '' }}>{!! $geo->tname !!}</option>
                                                  @endforeach
                                                </select>
                                            </div>
                                        </div>
              ...
              <div id="accordionWithIcon-2" class="accordion-collapse collapse show">
                <div class="accordion-body">
                  ...
                  <div class="row g-3">
                    <div class="row">
                    <input type="hidden" name="approval_status" id="required" value="{{old('approval_status', 'required')}}" />
                    <div class="row add_users" style="margin-top: 30px; display: {{ (old('approval_status', 'required') == 'required') ? 'block':'none'}}">


                        @php 
                            $approval_required_users = old('approval_required_users', []); 
                            $approval_required_type = old('approval_required_user_type', []); 
                            $approval_required_desg = old('approval_required_desg', []); 
                        @endphp
                        <input type="hidden" id="user_position"
                            value="{{ (count($approval_required_users) == 0) ? 1 : count($approval_required_users)  }}" />                                                    
                        @if (old('approval_status', 'auto') == 'required')
                        @foreach ($approval_required_type as $key => $approval_required_user_type_data)
                        @php
                        $approval_required_users_data = $approval_required_users[$key];
                        $approval_required_desig_data = $approval_required_desg[$key];
                        @endphp
                            ...
                            <div class="row" id="">
                                <div class="col-2">
                                   <select class="form-select approval_user_type" name="approval_required_user_type[]" data-row-sel="{{ $key + 1 }}">
                                    <option value="name" {{($approval_required_user_type_data ?? 'name') == 'name' ? 'selected' : ''}}>By Name</option>
                                    
                                  </select>                                
                                </div>
                                <div class="col {{ $approval_required_user_type_data == 'name' ? 'd-none' : '' }} by_name_desg_{{ $key + 1 }} by_designation_{{ $key + 1 }}">
                                   <select class="form-select user_type" name="approval_required_desg[]">
                                    <option value="branch_head" {{$approval_required_desig_data == 'branch_head' ? 'selected' : ''}}>Branch Head</option>
                                    <option value="branch_dep_head" {{$approval_required_desig_data == 'branch_dep_head' ? 'selected' : ''}}>Branch Dept Head</option>
                                    <option value="overall_dept_head" {{$approval_required_desig_data == 'overall_dept_head' ? 'selected' : ''}}>Over All Dept Head</option>
                                  </select>                                
                                </div>                                
                                <div class="col {{ $approval_required_user_type_data == 'designation' ? 'd-none' : '' }} select_users by_name_desg_{{ $key + 1 }} by_name_{{ $key + 1 }}">
                                    <select class="form-select users approval_user"
                                        ...
                                        <option data-ty="{{$approval_required_users_data}}"
                                            value="{{ $add_users_data->id }}:{{ $add_users_data->FirstName }}:{{$add_users_data->Email}}"
                                            {{ $approval_required_users_data == $add_users_data->id.":".$add_users_data->FirstName.":".$add_users_data->Email ? 'selected' : '' }}>
                                ...
                                @if($key == 0)
                                    <div class="col select_users_btn">
                                      ...
                                      <a href="javascript:;" class="nav-link btn justify-content-center user_add_row_party" data-mode="no_approve">
                        ...
                        <div class="user_row_1 repeater">
                          <div class="row" style="" id="">
                            ...
                            <div class="col select_users_btn">
                              <a href="javascript:;" class="nav-link btn justify-content-center user_add_row_party" data-tab-type="" data-mode="no_approve">
                                <div class="badge bg-label-success rounded p-2">
                                  <i class="ti ti-plus ti-sm"></i>
                                </div>
                              </a>
